...
class RTSAgent(nn.Module):
    ...
    def encode_state(self, agent_unit, friendly_units, enemy_units):
        ...
        if mass_spots:
            nearest_mass = min(mass_spots, key=lambda p: (p[0] - agent_unit['x'])**2 + (p[1] - agent_unit['z'])**2)
        ...
        # Map branch (summary stats from surrounding heights)
        heights = sample_surrounding_heights(agent_unit['x'], agent_unit['z'])
        if heights:
            heights_t = torch.tensor(heights, dtype=torch.float32, device=device)
            mean_h = torch.mean(heights_t)
            std_h = torch.std(heights_t, unbiased=False)
            min_h = torch.min(heights_t)
            max_h = torch.max(heights_t)
            median_h = torch.median(heights_t)
            range_h = max_h - min_h
            map_features = torch.stack([mean_h, std_h, min_h, max_h, median_h, range_h])
        else:
            map_features = torch.zeros(MAP_FEATURES_SIZE, dtype=torch.float32, device=device)
        map_emb = self.map_encoder(map_features)
        ...
        for u in enemy_units:
            enemy_vectors.append([
                ...
                u['z'] - agent_unit['z'],
                u['health'],
                u['range'] if 'range' in u else 0.0  # Include range if available
            ])
        if enemy_vectors:
        ...
        # Concatenate branch embeddings into a fixed-size state vector
        return torch.cat([self_emb, mass_emb, map_emb, friendly_emb, enemy_emb], dim=0)
...
def get_local_view_image(unit_x, unit_z, map_heights_array, view_size=250):
    ...
    unit_local_x = min(half, max(0, int((unit_x / 8) - x_min)))
    unit_local_z = min(half, max(0, int((unit_z / 8) - z_min)))
    ...
    for radius in [100, 200]:
        for angle in np.linspace(0, 2 * np.pi, num=6 if radius == 100 else 12, endpoint=False):
            sense_x = unit_x + radius * np.cos(angle)
            sense_z = unit_z + radius * np.sin(angle)
            sense_local_x = min(half, max(0, int((sense_x / 8) - x_min)))
            sense_local_z = min(half, max(0, int((sense_z / 8) - z_min)))
...
def get_state(agent_unit, friendly_units, enemy_units):
    ...
    return agent.encode_state(agent_unit, friendly_units, enemy_units)
...
def compute_reward(agent_unit, prev_health):
    ...
    enemy_range_image = generate_enemy_range_image(eUnits, map_width, map_height, map_heights.shape if map_heights is not None else None)
    if enemy_range_image is not None:
        map_x = int(agent_unit['x'] / 8)
        map_z = int(agent_unit['z'] / 8)
        if 0 <= map_z < enemy_range_image.shape[0] and 0 <= map_x < enemy_range_image.shape[1]:
            ...
            for unit in friendly_units:
                ...
                try:
                    if step_counter % 20 == 0:
                        local_view = get_local_view_image(unit['x'], unit['z'], map_heights, view_size=256)
